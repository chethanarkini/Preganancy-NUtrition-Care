...
def save_model(model, model_path='chat_model.h5'):  # Changed file extension to HDF5 format
    model.save(model_path)

def save_tokenizer(tokenizer, tokenizer_path='tokenizer.pickle'):
    with open(tokenizer_path, 'wb') as handle:
        pickle.dump(tokenizer, handle, protocol=pickle.HIGHEST_PROTOCOL)

def save_label_encoder(label_encoder, encoder_path='label_encoder.pickle'):
    with open(encoder_path, 'wb') as enc:
...
if __name__ == '__main__':
    ...
    model, history = train_model(model, padded_sequences, training_labels)
    model.summary()
    save_model(model)
